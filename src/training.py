...
class ContrastiveLoss(torch.nn.Module):
    ...
    def forward(self, output1, output2, output3, target,size_average=True):
        target = target.to(self.device)
        distances = (output1 + output2 - output3).pow(2).sum(1).to(self.device)  # squared distances
    ...
    for epoch in range(EPOCH):
        ...
        for step, input_seq in enumerate(data_train):
            for adm_idx, adm in enumerate(input_seq):
                ...
                result, loss_ddi, G_s, G_e, G_i = model(
                    patient_data=input_seq[:adm_idx + 1],
                    **drug_data
                )
                ...
                loss_g = contra_loss(G_s, G_e, G_i, torch.FloatTensor(bce_target).to(device))
                ...
                if current_ddi_rate <= target_ddi:
                    loss = 0.90 * loss_bce + 0.05 * loss_multi + 0.05 * loss_g
                else:
                    beta = coef * (1 - (current_ddi_rate / target_ddi))
                    beta = min(math.exp(beta), 1)
                    loss = beta * (0.90 * loss_bce + 0.05 * loss_multi + 0.05 * loss_g) \
                        + (1 - beta) * loss_ddi
